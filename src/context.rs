...
impl Context {
    ...
    pub fn make_undefined(&self) -> JSValueRef {
        let value = unsafe { value::JS_MKVAL_real(sys::JS_TAG_UNDEFINED, 0) };
        JSValueRef::from_js_value(self.0, value)
    }

    pub fn make_bool(&self, flag: bool) -> JSValueRef {
        let value = unsafe { value::JS_MKVAL_real(sys::JS_TAG_BOOL, if flag { 1 } else { 0 }) };
        JSValueRef::from_js_value(self.0, value)
    }

    pub fn make_null(&self) -> JSValueRef {
        let value = unsafe { value::JS_MKVAL_real(sys::JS_TAG_NULL, 0) };
        JSValueRef::from_js_value(self.0, value)
    }

    pub fn make_int(&self, value: i32) -> JSValueRef {
        let value = unsafe { value::JS_MKVAL_real(sys::JS_TAG_INT, value) };
        JSValueRef::from_js_value(self.0, value)
    }

    pub fn make_float(&self, value: f64) -> JSValueRef {
        let value = unsafe { value::JS_NewFloat64_real(self.0, value) };
        JSValueRef::from_js_value(self.0, value)
    }
